...
# Espera hasta que el cliente esté listo, buscando texto en la pantalla
...
def iniciar_sesion():
    ...
    try:
        ...
        time.sleep(7)  # Esperar a que el cliente se abra

        app = pywinauto.Application(backend="uia").connect(title="Riot Client")  # Conectar con la ventana del juego

        # Esperar que la ventana esté visible
        ventana = app.window(title="Riot Client")
        ventana.wait('visible')  # Espera a que la ventana sea visible
        if ventana.exists():
            ventana.set_focus()  # Poner la ventana en primer plano
            pyautogui.write(usuario)  # Escribir el nombre de usuario
            pyautogui.press('tab')  # Presionar tab para ir al campo de contraseña
            pyautogui.write(contrasena)  # Escribir la contraseña
            pyautogui.press('enter')  # Presionar enter para iniciar sesión
        else:
            messagebox.showerror("Error", "No se pudo encontrar la ventana del juego.")
